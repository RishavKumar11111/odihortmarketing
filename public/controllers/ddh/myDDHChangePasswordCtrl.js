app.controller('myDDHChangePasswordCtrl', function ($scope, $http, $filter) {
        ...
        if (isValid) {
            ...
            if (sha256($scope.txtOldPassword) !== obj.NewPassword) {
                if (obj.NewPassword === obj.ConfirmPassword) {
                    $http.post('http://localhost:3000/ddh/changePassword', { data: obj }, { credentials: 'same-origin', headers: { 'CSRF-Token': token } }).then(function success(response) {
